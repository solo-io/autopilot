...
func TestIstioObserver_GetRequestSuccessRate(t *testing.T) {
	expected := `sum(rate(istio_requests_total{5mdestination_workload_namespace="default",destination_workload=~"podinfo",response_code!~"5.*"}[1m]))/sum(rate(istio_requests_total{5mdestination_workload_namespace="default",destination_workload=~"podinfo"}[1m]))*100`
...
func TestIstioObserver_GetRequestDuration(t *testing.T) {
	expected := `histogram_quantile(0.99,sum(rate(istio_request_duration_seconds_bucket{5mdestination_workload_namespace="default",destination_workload=~"podinfo"}[1m]))by(le))`
