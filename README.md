...
func main() {
	...
	// some in-process options for our guy?
	...
	if err := generated_run.Run(canaryHandler, fooHandler); err != nil {
...
cluster-agents (must run in the same cluster as operator):
    - option to run in the operator as a library
- cross-cluster garbage collector
- cross-cluster service account importer
